((QUnit => {
  ...
  QUnit.module('Pagebone.Collection', {

    beforeEach(assert) {
      a         = new Pagebone.Model({id: 3, label: 'a'});
      b         = new Pagebone.Model({id: 2, label: 'b'});
      c         = new Pagebone.Model({id: 1, label: 'c'});
      d         = new Pagebone.Model({id: 0, label: 'd'});
      e         = null;
      col       = new Pagebone.Collection([a, b, c, d]);
      otherCol  = new Pagebone.Collection();
    }
  ...
  QUnit.test('String comparator.', assert => {
    assert.expect(1);
    var collection = new Pagebone.Collection([
      {id: 3},
      {id: 1},
      {id: 2}
    ], {comparator: 'id'});
  ...
  QUnit.test('new and parse', assert => {
    assert.expect(3);
    var Collection = Pagebone.Collection.extend({
      parse(data) {
  ...
  QUnit.test('clone preserves model and comparator', assert => {
    assert.expect(3);
    var Model = Pagebone.Model.extend();
    var comparator = model => model.id;

    var collection = new Pagebone.Collection([{id: 1}], {
      model: Model,
      comparator
    }).clone();
    collection.add({id: 2});
  ...
  QUnit.test('get with non-default ids', assert => {
    assert.expect(5);
    var MongoModel = Pagebone.Model.extend({idAttribute: '_id'});
    var model = new MongoModel({_id: 100});
    var collection = new Pagebone.Collection([model], {model: MongoModel});
    ...
    var collection2 = new Pagebone.Collection();
  ...
  QUnit.test('has', assert => {
    ...
    var outsider = new Pagebone.Model({id: 4});
  ...
  QUnit.test('add', assert => {
    ...
    e = new Pagebone.Model({id: 10, label: 'e'});
    otherCol.add(e);
    otherCol.on('add', () => {
      secondAdded = true;
    });
    ...
    var f = new Pagebone.Model({id: 20, label: 'f'});
    var g = new Pagebone.Model({id: 21, label: 'g'});
    var h = new Pagebone.Model({id: 22, label: 'h'});
    var atCol = new Pagebone.Collection([f, g, h]);
    ...
    var coll = new Pagebone.Collection(new Array(2));
    var addCount = 0;
    coll.on('add', () => {
      addCount += 1;
    });
  ...
  QUnit.test('add multiple models', assert => {
    assert.expect(6);
    var collection = new Pagebone.Collection([{at: 0}, {at: 1}, {at: 9}]);
  ...
  QUnit.test('add; at should have preference over comparator', assert => {
    assert.expect(1);
    var Col = Pagebone.Collection.extend({
      comparator(m1, m2) {
    ...
    collection.add(new Pagebone.Model({id: 1}), {at: 1});
  ...
  QUnit.test('add; at should add to the end if the index is out of bounds', assert => {
    assert.expect(1);
    var collection = new Pagebone.Collection([{id: 2}, {id: 3}]);
    collection.add(new Pagebone.Model({id: 1}), {at: 5});

    assert.equal(collection.pluck('id').join(' '), '2 3 1');
  });

  QUnit.test("can't add model to collection twice", assert => {
    var collection = new Pagebone.Collection([{id: 1}, {id: 2}, {id: 1}, {id: 2}, {id: 3}]);
  ...
  QUnit.test('add model to multiple collections', assert => {
    assert.expect(10);
    var counter = 0;
    var m = new Pagebone.Model({id: 10, label: 'm'});
    ...
    var col1 = new Pagebone.Collection([]);
    col1.on('add', (model, collection) => {
      assert.equal(m, model);
      assert.equal(col1, collection);
    });
    var col2 = new Pagebone.Collection([]);
  ...
  QUnit.test('add model with parse', assert => {
    ...
    var Col = Pagebone.Collection.extend({model: Model});
  ...
  QUnit.test('add with parse and merge', assert => {
    var collection = new Pagebone.Collection();
    collection.parse = attrs => _.map(attrs, model => {
  ...
  QUnit.test('add model to collection with sort()-style comparator', assert => {
    assert.expect(3);
    var collection = new Pagebone.Collection;
    collection.comparator = (m1, m2) => m1.get('name') < m2.get('name') ? -1 : 1;
    var tom = new Pagebone.Model({name: 'Tom'});
    var rob = new Pagebone.Model({name: 'Rob'});
    var tim = new Pagebone.Model({name: 'Tim'});
    collection.add(tom);
    collection.add(rob);
    collection.add(tim);
  ...
  QUnit.test('add and remove return values', assert => {
    assert.expect(13);
    var Even = Pagebone.Model.extend({
      validate(attrs) {
    ...
    assert.ok(list[0] instanceof Pagebone.Model);
  ...
  QUnit.test('shift and pop', assert => {
    assert.expect(2);
    var collection = new Pagebone.Collection([{a: 'a'}, {b: 'b'}, {c: 'c'}]);
  ...
  QUnit.test('events are unbound on remove', assert => {
    assert.expect(3);
    var counter = 0;
    var dj = new Pagebone.Model();
    var emcees = new Pagebone.Collection([dj]);
  ...
  QUnit.test('remove in multiple collections', assert => {
    ...
    var passed = false;
    var m1 = new Pagebone.Model(modelData);
    var m2 = new Pagebone.Model(modelData);
    m2.on('remove', () => {
      passed = true;
    });
    var col1 = new Pagebone.Collection([m1]);
    var col2 = new Pagebone.Collection([m2]);
  ...
  QUnit.test('remove same model in multiple collection', assert => {
    assert.expect(16);
    var counter = 0;
    var m = new Pagebone.Model({id: 5, title: 'Othello'});
    ...
    var col1 = new Pagebone.Collection([m]);
    col1.on('remove', (model, collection) => {
      assert.equal(m, model);
      assert.equal(col1, collection);
    });
    var col2 = new Pagebone.Collection([m]);
  ...
  QUnit.test('Collection: non-persisted model destroy removes from all collections', assert => {
    assert.expect(3);
    var m = new Pagebone.Model({title: 'Othello'});
    m.sync = (method, model, options) => { throw 'should not be called'; };
    var col1 = new Pagebone.Collection([m]);
    var col2 = new Pagebone.Collection([m]);
    m.destroy();
    assert.ok(col1.length === 0);
  ...
  QUnit.test('#3283 - fetch with an error response calls error with context', assert => {
    ...
    collection.sync = (method, model, opts) => {
  ...
  QUnit.test('create with validate:true enforces validation', assert => {
    assert.expect(3);
    var ValidatingModel = Pagebone.Model.extend({
      validate(attrs) {
        return 'fail';
      }
    });
    var ValidatingCollection = Pagebone.Collection.extend({
  ...
  QUnit.test('create will pass extra options to success callback', function(assert) {
    ...
    var Model = Pagebone.Model.extend({
      sync(method, model, options) {
        ...
        return Pagebone.Model.prototype.sync.call(this, method, model, options);
      }
    });
    ...
    var success = (model, response, options) => {
  ...
  QUnit.test('preinitialize occurs before the collection is set up', assert => {
    ...
    var FooModel = Pagebone.Model.extend({id: 'foo'});
    var coll = new Collection({}, {
      model: FooModel
    });
  ...
  QUnit.test('where and findWhere', assert => {
    assert.expect(8);
    var model = new Pagebone.Model({a: 1});
    var coll = new Pagebone.Collection([
      model,
      {a: 1},
      {a: 1, b: 2},
      {a: 2, b: 2},
  ...
  QUnit.test('Underscore methods with object-style and property-style iteratee', assert => {
    assert.expect(26);
    var model = new Pagebone.Model({a: 4, b: 1, e: 3});
    var coll = new Pagebone.Collection([
      {a: 1, b: 1},
      {a: 2, b: 1, c: 1},
      {a: 3, b: 1},
  ...
  QUnit.test('same references in reset', assert => {
    var model = new Pagebone.Model({id: 1});
    var collection = new Pagebone.Collection({id: 1});
  ...
  QUnit.test('reset passes caller options', assert => {
    ...
    var collection = new (Pagebone.Collection.extend({model: Model}))();
  ...
  QUnit.test('add does not alter arguments', assert => {
    ...
    var attrs = {};
    var models = [attrs];
    new Pagebone.Collection().add(models);
  ...
  QUnit.test('Invalid models are discarded with validate:true.', assert => {
    ...
    collection.model = Pagebone.Model.extend({
  ...
  QUnit.test('#1112 - passing options.model sets collection.model', assert => {
    assert.expect(2);
    var Model = Pagebone.Model.extend({});
    var collection = new Pagebone.Collection([{id: 1}], {model: Model});
  ...
  QUnit.test('null and undefined are invalid ids.', assert => {
    assert.expect(2);
    var model = new Pagebone.Model({id: 1});
    var collection = new Pagebone.Collection([model]);
    model.set({id: null});
  ...
  QUnit.test('#1355 - `options` is passed to success callbacks', assert => {
    ...
    collection.sync = m.sync = (method, coll, options) => {
      options.success({});
    };
  ...
  QUnit.test("#1412 - Trigger 'request' and 'sync' events.", assert => {
    ...
    var collection = new Pagebone.Collection;
    collection.url = '/test';
    Pagebone.ajax = settings => { settings.success(); };
  ...
  QUnit.test('#1655 - groupBy can be used with a string argument.', assert => {
    assert.expect(3);
    var collection = new Pagebone.Collection([{x: 1}, {x: 2}]);
  ...
  QUnit.test('#1655 - sortBy can be used with a string argument.', assert => {
    assert.expect(1);
    var collection = new Pagebone.Collection([{x: 3}, {x: 1}, {x: 2}]);
    var values = _.map(collection.sortBy('x'), model => model.get('x'));
  ...
  QUnit.test("`sort` shouldn't always fire on `add`", assert => {
    assert.expect(1);
    var collection = new Pagebone.Collection([{id: 1}, {id: 2}, {id: 3}], {
      comparator: 'id'
    });
  ...
  QUnit.test('set', assert => {
    var m1 = new Pagebone.Model();
    var m2 = new Pagebone.Model({id: 2});
    var m3 = new Pagebone.Model();
    var collection = new Pagebone.Collection([m1, m2]);
  ...
  QUnit.test('set + merge with default values defined', assert => {
    ...
    var collection = new Pagebone.Collection([m], {model: Model});
  ...
  QUnit.test('merge without mutation', assert => {
    ...
    var Collection = Pagebone.Collection.extend({model: Model});
  ...
  QUnit.test('`set` matches input order in the absence of a comparator', assert => {
    var one = new Pagebone.Model({id: 1});
    var two = new Pagebone.Model({id: 2});
    var three = new Pagebone.Model({id: 3});
    var collection = new Pagebone.Collection([one, two, three]);
  ...
  QUnit.test('`set` with non-normal id', assert => {
    var Collection = Pagebone.Collection.extend({
      model: Pagebone.Model.extend({idAttribute: '_id'})
    });
  ...
  QUnit.test('#1915 - `parse` data in the right order in `set`', assert => {
    var collection = new (Pagebone.Collection.extend({
      parse(data) {
  ...
  QUnit.test('fetch will pass extra options to success callback', function(assert) {
    assert.expect(1);
    var SpecialSyncCollection = Pagebone.Collection.extend({
      url: '/test',
      sync(method, collection, options) {
        _.extend(options, {specialSync: true});
        return Pagebone.Collection.prototype.sync.call(this, method, collection, options);
      }
    });

    var collection = new SpecialSyncCollection();

    var onSuccess = (coll, resp, options) => {
      assert.ok(options.specialSync, 'Options were passed correctly to callback');
    };
  ...
  QUnit.test('#2612 - nested `parse` works with `Collection#set`', assert => {
    ...
    var Job = Pagebone.Model.extend({
      constructor() {
        this.items = new Items();
        Pagebone.Model.apply(this, arguments);
      },
      parse(attrs) {
    ...
    var Item = Pagebone.Model.extend({
      constructor() {
        this.subItems = new Pagebone.Collection();
        Pagebone.Model.apply(this, arguments);
      },
      parse(attrs) {
    ...
    var Items = Pagebone.Collection.extend({
      model: Item
    });
  ...
  QUnit.test('_addReference binds all collection events & adds to the lookup hashes', assert => {
    ...
    var Collection = Pagebone.Collection.extend({
      ...
      _addReference(model) {
        Pagebone.Collection.prototype._addReference.apply(this, arguments);
        calls.add++;
      ...
      _removeReference(model) {
        Pagebone.Collection.prototype._removeReference.apply(this, arguments);
        calls.remove++;
  ...
  QUnit.test('modelId', assert => {
    var Stooge = Pagebone.Model.extend();
    var StoogeCollection = Pagebone.Collection.extend({model: Stooge});
  ...
  QUnit.test('Polymorphic models work with "simple" constructors', assert => {
    var A = Pagebone.Model.extend();
    var B = Pagebone.Model.extend();
    var C = Pagebone.Collection.extend({
      model(attrs) {
  ...
  QUnit.test('Polymorphic models work with "advanced" constructors', assert => {
    var A = Pagebone.Model.extend({idAttribute: '_id'});
    var B = Pagebone.Model.extend({idAttribute: '_id'});
    var C = Pagebone.Collection.extend({
      model: Pagebone.Model.extend({
        constructor(attrs) {
          return attrs.type === 'a' ? new A(attrs) : new B(attrs);
  ...
  QUnit.test('Collection with polymorphic models receives default id from modelId', assert => {
    ...
    var C1 = Pagebone.Collection.extend({
      model(attrs) {
        return new Pagebone.Model(attrs);
      }
    });
    var c1 = new C1({id: 1});
    ...
    var M = Pagebone.Model.extend({
      idAttribute: '_id'
    });
    var C2 = Pagebone.Collection.extend({
      model(attrs) {
        return new M(attrs);
      }
    });
  ...
  QUnit.test('#3039 #3951: adding at index fires with correct at', assert => {
    assert.expect(4);
    var collection = new Pagebone.Collection([{val: 0}, {val: 4}]);
  ...
  QUnit.test('#3039: index is not sent when at is not specified', assert => {
    assert.expect(2);
    var collection = new Pagebone.Collection([{at: 0}]);
    collection.on('add', (model, coll, options) => {
  ...
  QUnit.test('#set accepts options.at as a string', assert => {
    assert.expect(1);
    var collection = new Pagebone.Collection([{id: 1}, {id: 2}]);
  ...
  QUnit.test("#3711 - remove's `update` event returns one removed model", assert => {
    var model = new Pagebone.Model({id: 1, title: 'First Post'});
    var collection = new Pagebone.Collection([model]);
  ...
  QUnit.test("#3711 - remove's `update` event returns multiple removed models", assert => {
    var model = new Pagebone.Model({id: 1, title: 'First Post'});
    var model2 = new Pagebone.Model({id: 2, title: 'Second Post'});
    var collection = new Pagebone.Collection([model, model2]);
    collection.on('update', (context, options) => {
      var changed = options.changes;
      assert.deepEqual(changed.added, []);
      assert.deepEqual(changed.merged, []);
  ...
  QUnit.test("#3711 - set's `update` event returns one removed model", assert => {
    var model = new Pagebone.Model({id: 1, title: 'First Post'});
    var model2 = new Pagebone.Model({id: 2, title: 'Second Post'});
    var model3 = new Pagebone.Model({id: 3, title: 'My Last Post'});
    var collection = new Pagebone.Collection([model]);
    collection.on('update', (context, options) => {
      var changed = options.changes;
  ...
  QUnit.test("#3711 - set's `update` event returns one merged model", assert => {
    var model = new Pagebone.Model({id: 1, title: 'First Post'});
    var model2 = new Pagebone.Model({id: 2, title: 'Second Post'});
    var model2Update = new Pagebone.Model({id: 2, title: 'Second Post V2'});
    var collection = new Pagebone.Collection([model, model2]);
    collection.on('update', (context, options) => {
  ...
  QUnit.test("#3711 - set's `update` event returns multiple merged models", assert => {
    var model = new Pagebone.Model({id: 1, title: 'First Post'});
    var modelUpdate = new Pagebone.Model({id: 1, title: 'First Post V2'});
    var model2 = new Pagebone.Model({id: 2, title: 'Second Post'});
    var model2Update = new Pagebone.Model({id: 2, title: 'Second Post V2'});
    var collection = new Pagebone.Collection([model, model2]);
    collection.on('update', (context, options) => {
