((QUnit => {
  ...
  var ProxyModel = Pagebone.Model.extend();
  var Klass = Pagebone.Collection.extend({
    url() { return '/collection'; }
  });
  var doc, collection;

  QUnit.module('Pagebone.Model', {
  ...
  QUnit.test('Object.prototype properties are overridden by attributes', assert => {
    assert.expect(1);
    var model = new Pagebone.Model({hasOwnProperty: true});
  ...
  QUnit.test('preinitialize occurs before the model is set up', assert => {
    ...
    var model = new Model({id: 'foo'}, {collection});
    assert.equal(model.collection, collection);
  ...
  QUnit.test('underscore methods', assert => {
    assert.expect(5);
    var model = new Pagebone.Model({foo: 'a', bar: 'b', baz: 'c'});
  ...
  QUnit.test('chain', assert => {
    var model = new Pagebone.Model({a: 0, b: 1, c: 2});
    assert.deepEqual(model.chain().pick('a', 'b', 'c').values().compact().value(), [1, 2]);
  });

  QUnit.test('clone', assert => {
    assert.expect(10);
    var a = new Pagebone.Model({foo: 1, bar: 2, baz: 3});
    var b = a.clone();
    ...
    var foo = new Pagebone.Model({p: 1});
    var bar = new Pagebone.Model({p: 2});
  ...
  QUnit.test('isNew', assert => {
    ...
    a = new Pagebone.Model({foo: 1, bar: 2, baz: 3, id: -5});
    assert.ok(!a.isNew(), 'any defined ID is legal, negative or positive');
    a = new Pagebone.Model({foo: 1, bar: 2, baz: 3, id: 0});
    assert.ok(!a.isNew(), 'any defined ID is legal, including zero');
    assert.ok(new Pagebone.Model().isNew(), 'is true when there is no id');
    assert.ok(!new Pagebone.Model({id: 2}).isNew(), 'is false for a positive integer');
    assert.ok(!new Pagebone.Model({id: -5}).isNew(), 'is false for a negative integer');
  });
  ...
  QUnit.test('matches with predicate', assert => {
    var model = new Pagebone.Model({a: 0});
  ...
  QUnit.test('set and unset', assert => {
    assert.expect(8);
    var a = new Pagebone.Model({id: 'id', foo: 1, bar: 2, baz: 3});
    var changeCount = 0;
  ...
  QUnit.test('set falsy values in the correct order', assert => {
    assert.expect(2);
    var model = new Pagebone.Model({result: 'result'});
    model.on('change', () => {
  ...
  QUnit.test('set an empty string', assert => {
    assert.expect(1);
    var model = new Pagebone.Model({name: 'Model'});
    model.set({name: ''});
  ...
  QUnit.test('clear', assert => {
    assert.expect(3);
    var changed;
    var model = new Pagebone.Model({id: 1, name: 'Model'});
  ...
  QUnit.test('defaults', assert => {
    assert.expect(9);
    var Defaulted = Pagebone.Model.extend({
      defaults: {
        one: 1,
        two: 2
      }
    ...
    Defaulted = Pagebone.Model.extend({
      defaults() {
        return {
          one: 3,
          two: 4
  ...
  QUnit.test('change, hasChanged, changedAttributes, previous, previousAttributes', assert => {
    assert.expect(9);
    var model = new Pagebone.Model({name: 'Tim', age: 10});
  ...
  QUnit.test('changedAttributes', assert => {
    assert.expect(3);
    var model = new Pagebone.Model({a: 'a', b: 'b'});
    assert.deepEqual(model.changedAttributes(), false);
  ...
  QUnit.test('change with options', assert => {
    assert.expect(2);
    var value;
    var model = new Pagebone.Model({name: 'Rob'});
  ...
  QUnit.test('change after initialize', assert => {
    ...
    var obj = new Pagebone.Model(attrs);
  ...
  QUnit.test('save within change event', function(assert) {
    assert.expect(1);
    var env = this;
    var model = new Pagebone.Model({firstName: 'Taylor', lastName: 'Swift'});
    model.url = '/test';
    model.on('change', () => {
      model.save();
  ...
  QUnit.test('validate after save', assert => {
    assert.expect(2);
    var lastError, model = new Pagebone.Model();
  ...
  QUnit.test('save will pass extra options to success callback', function(assert) {
    assert.expect(1);
    var SpecialSyncModel = Pagebone.Model.extend({
      sync(method, m, options) {
        _.extend(options, {specialSync: true});
        return Pagebone.Model.prototype.sync.call(this, method, m, options);
      },
      urlRoot: '/test'
    });
    ...
    var onSuccess = (m, response, options) => {
  ...
  QUnit.test('destroy', function(assert) {
    ...
    var newModel = new Pagebone.Model;
  ...
  QUnit.test('non-persisted destroy', assert => {
    assert.expect(1);
    var a = new Pagebone.Model({foo: 1, bar: 2, baz: 3});
    a.sync = () => { throw 'should not be called'; };
    a.destroy();
  ...
  QUnit.test('validate on unset and clear', assert => {
    assert.expect(6);
    var error;
    var model = new Pagebone.Model({name: 'One'});
    model.validate = attrs => {
      if (!attrs.name) {
        error = true;
        return 'No thanks.';
  ...
  QUnit.test('Inherit class properties', assert => {
    assert.expect(6);
    var Parent = Pagebone.Model.extend({
      instancePropSame() {},
      instancePropDiff() {}
    }, {
      classProp() {}
  ...
  QUnit.test('hasChanged/set should use same comparison', assert => {
    assert.expect(2);
    var changed = 0, model = new Pagebone.Model({a: null});
  ...
  QUnit.test('set value regardless of equality/change', assert => {
    assert.expect(1);
    var model = new Pagebone.Model({x: []});
    var a = [];
    model.set({x: a});
  ...
  QUnit.test('set same value does not trigger change', assert => {
    assert.expect(0);
    var model = new Pagebone.Model({x: 1});
  ...
  QUnit.test('unset does not fire a change for undefined attributes', assert => {
    assert.expect(0);
    var model = new Pagebone.Model({x: undefined});
  ...
  QUnit.test('`previous` for falsey keys', assert => {
    assert.expect(2);
    var model = new Pagebone.Model({'0': true, '': true});
  ...
  QUnit.test('`save` with `wait` sends correct attributes', function(assert) {
    assert.expect(5);
    var changed = 0;
    var model = new Pagebone.Model({x: 1, y: 2});
    model.url = '/test';
  ...
  QUnit.test('#1943 change calculations should use _.isEqual', assert => {
    var model = new Pagebone.Model({a: {key: 'value'}});
  ...
  QUnit.test('isValid', assert => {
    var model = new Pagebone.Model({valid: true});
    model.validate = attrs => {
