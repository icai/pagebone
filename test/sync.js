((QUnit => {
  ...
  var Library = Pagebone.Collection.extend({
    url() { return '/library'; }
  });
  var library;
  ...
  QUnit.test('#1052 - `options` is optional.', assert => {
    ...
    var model = new Pagebone.Model();
    model.url = '/test';
    Pagebone.sync('create', model);
  });

  QUnit.test('Pagebone.ajax', assert => {
    assert.expect(1);
  ...
  QUnit.test('Call provided error callback on error.', function(assert) {
    ...
    var model = new Pagebone.Model;
    model.url = '/test';
    Pagebone.sync('read', model, {
      error() { assert.ok(true); }
    });
    this.ajaxSettings.error();
  });

  QUnit.test('Use Pagebone.emulateHTTP as default.', function(assert) {
    assert.expect(2);
    var model = new Pagebone.Model;
    model.url = '/test';

    Pagebone.emulateHTTP = true;
  ...
  QUnit.test('Use Pagebone.emulateJSON as default.', function(assert) {
    assert.expect(2);
    var model = new Pagebone.Model;
    model.url = '/test';

    Pagebone.emulateJSON = true;
    model.sync('create', model);
